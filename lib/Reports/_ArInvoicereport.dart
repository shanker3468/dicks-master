...
class ARInvoiceReport extends StatefulWidget {
  ARInvoiceReport({Key? key, required this. screenName}) : super(key: key);
  var screenName='';

  @override
  State<ARInvoiceReport> createState() => _ARInvoiceReportState();
}

class _ARInvoiceReportState extends State<ARInvoiceReport> {
  ...
  Future getSalesOrderRecord(Status) async {
    ...
    var header = {"Content-Type": "application/json"};
    var responce = await http.post(
        Uri.parse('${AppConstants.LIVE_URL}getarinvoice'),
        body: json.encode(body),
        headers: header);
    log('${AppConstants.LIVE_URL}getarinvoice');
    //log(responce.body);
    if (responce.statusCode == 200) {
      final decode = jsonDecode(responce.body);
      if(decode['status']==0){
